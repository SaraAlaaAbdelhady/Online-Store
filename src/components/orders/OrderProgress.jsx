...
function OrderProgress({ status }) {
    ...
    return(
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 mb-6 p-5">
            {/* header */}
            <h2 className="text-slate-800 dark:text-slate-200 text-lg font-semibold">Order Progress</h2>

            {/* progress bar */}
            <div className="relative flex justify-between mt-4 md:mt-6">
                {/* gray track */}
                <div className="absolute top-4 left-4 right-4 h-0.5 bg-slate-200 dark:bg-slate-700"></div>
                {/* blue track */}
                <div 
                ...
                {progressSteps.map((step, index) => {
                    ...
                    return (
                        <div key={step} className="relative z-10 flex flex-col items-center w-8">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isReached ? "bg-blue-500 text-white" : "bg-slate-300 text-slate-400 dark:bg-slate-700"}`}>
                                {isReached ? <IoMdCheckmarkCircleOutline size={18} /> : <MdOutlineCircle size={18} />}
                            </div>
                            <span className={`text-xs mt-2 font-medium whitespace-nowrap ${isReached ? "text-blue-600" : "text-slate-600 dark:text-slate-400"}`}>{step}</span>
                        </div>
                    );
                })}
            </div>
